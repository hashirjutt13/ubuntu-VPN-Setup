...
On each run, the script cleans the previous WireGuard/wstunnel setup, removes old generated client files, and frees TCP `443` if it is being used by common web services such as `nginx`, `apache2`, `caddy`, `haproxy`, or `traefik`.
...
After the script finishes, it prints the key Xlarva values and saves the full set at:

```bash
sudo cat /root/xlarva-values.txt
```

The important values are:

- Server URL: `wss://YOUR_SERVER_IP:443`
- HTTP upgrade path prefix: generated by the script
- Local UDP listener: `127.0.0.1:51820`
- WireGuard endpoint: `127.0.0.1:51820`
- TLS verification: off / allow self-signed

For AWS, your EC2 Security Group must allow inbound TCP `443`. You do not need to open UDP `51820` for wstunnel mode.
